Shader "Procedural3D/UnlitGridBW3D"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert(appdata v)
			{
				...
				o.SSNormal = mul(UNITY_MATRIX_V, v.normal);
				return o;
			}
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				Grid3DAA(i.WSPos.xyz, ddposX, ddposY, _TileSize.xyz, edge, intensity);
				fixed4 col = float4(intensity, intensity, intensity, 1.0);
				//fixed4 col = float4(ddposX, 1.0);
